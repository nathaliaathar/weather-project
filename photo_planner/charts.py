...
def photography_score_chart(
    ...
    *,
    ...
    Clock times on the x-axis. Missing scores stay as gaps (not 0).
    """
    _ = window_end  # kept so callers can pass end; shading uses start + slot count
...
def city_scores_heatmap(
    ...
    *,
    ...
    # Absolute 0–100 colours: orange (low) → blue (high).
    fig = go.Figure(
        data=go.Heatmap(
            z=z,
            x=hour_columns,
