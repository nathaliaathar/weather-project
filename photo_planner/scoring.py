...
# Caps: if one factor is bad, the final score cannot stay very high.
# Example: final = min(base, 25 + 75*light, 35 + 65*rain, ...)
...
# Whole session: 60% average of the hours + 40% the weaker part
# (20th percentile). One rough hour still pulls the booking down.
...
# ------------------------------------------------------------
# Sun height from lat/lon + UTC time (not a weather API field)
# ------------------------------------------------------------
def sun_height_degrees(latitude: float, longitude: float, timestamp_unix: int) -> tuple[float, bool]:
    """
    Approximate sun height in degrees, plus whether it is afternoon.

    90 = overhead, 0 = on the horizon, negative = below.
    We use this for light quality and to skip dark pre-sunrise slots.
    Formula adapted from common solar-position math (same idea as
...
def factor_caps(light: float, rain: float, wind: float, comfort: float) -> dict[str, float]:
    """Upper limits so one bad factor can drag the whole score down."""
    return {
...
class HourScore:
    ...
    def __iter__(self):
        # Lets charts unpack each item as (hour, score).
        yield self.hour
        yield self.score
